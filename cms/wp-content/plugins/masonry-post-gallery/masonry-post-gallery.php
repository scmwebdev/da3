...
/* TO DO:
	...
	- Copy both CSS to dev
	- Compress both CSS
	- Rebuild shortcode gen
	- Start removing masonry off mode support	
	*/

class Cactus_Masonry
{	
	private static $id = "CM_GALLERY_";
	private static $CM_version = "0.5.0.0b";
	...
	<div class='CM_area' data-plugin='Cactus Masonry' data-version='" . self::$CM_version . "'>" . self::cmpg_create_styles() . "
		...
		//Parse post IDs
		$post_IDs = explode(",", self::$a['post_ids']);
		//Error check post IDs array
		$i_size = count($post_IDs);
		$post_filter = array();
		for($i = 0; $i < $i_size; $i++) {
			$temp = intval($post_IDs[$i]);
			if($temp > 0) array_push($post_filter, $temp);
		}
		...
		foreach($lastposts as $post) {
			setup_postdata($post);
			$id_ok = true;
			$i_size = count($post_filter);
			if($i_size > 0) {
				$id_ok = false;
				for($i = 0; $i < $i_size; $i++) {
					if($post->ID == $post_filter[$i]) {
						$id_ok = true;
						break;
					}
				}
			...
			if((has_post_thumbnail($post->ID) || !(self::$a['default_image_id'] === false))) {	
				...
				if($id_ok && self::$post_count >= self::$a['search_start'] && self::$post_count < self::$a['search_start']+self::$a['page_size']) $script_text .= self::render_post();
				self::$post_count++;
			}	
		}
	...
	static private function render_post() {
		...
		if($show_databox) {
			...
			if(self::$a['display_post_excerpts'] && strlen($excerpt) > 0) $data_text .= "<div class='cm_exerpt cm_excerpt'>{$excerpt}</div>";
			$data_text .= "</div>";	
		}
